...
import { promisify, type Callback } from './index.js';
...
describe('promisify', () => {
    it('should add a promise to a callback function', async () => {
        const test = promisify((cb: (err: Error | null, value?: number) => void) => {
            cb(null, 7);
        });
    ...
    it('should handle a function that throws synchronously', async () => {
        const test = promisify((_cb: (err: Error | null, value?: number) => void) => {
    ...
    it('should handle this in class method', async () => {
        class Foo {
            ...
            fn = promisify(function (this: Foo, cb: Callback<number>) {
                cb(null, this.value);
            });
        }
        const foo = new Foo();
    ...
    it('should handle this in class arrow method', async () => {
        class Foo {
            ...
            fn = promisify((cb: Callback<number>) => cb(null, this.value));
        }
        const foo = new Foo();
    ...
    it('should work with bind', async () => {
        ...
        const fn = promisify(function (this: { value: number }, cb: Callback<number>) {
            cb(null, this.value);
        }).bind(obj);
